...
class MailsterPiwikAnalytics {
	...
	public function redirect_to( $target, $campaign_id ) {
		...
		$values = wp_parse_args(get_post_meta( $campaign->ID, 'mailster-piwik', true ), mailster_option('piwik', array(
	...
	public function save_post( $post_id, $post ) {
		...
		if ( isset( $_POST['mailster_piwik'] ) && $post->post_type == 'newsletter' ) {
			...
			$piwik_values = mailster_option('piwik', array(
				'pk_campaign' => '%%CAMP_TITLE%%',
				'pk_kwd' => '%%LINK%%',
			));
	...
	public function metabox() {
		...
		$values = wp_parse_args(get_post_meta( $post->ID, 'mailster-piwik', true ), mailster_option('piwik', array(
			'pk_campaign' => '%%CAMP_TITLE%%',
			'pk_kwd' => '%%LINK%%',
		)));
